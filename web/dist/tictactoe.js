...
(() => {
  ...
  var TicTacToeGame = class _TicTacToeGame {
    ...
    performMove(move) {
      const [y2, x2] = move;
      const row = this.board[y2];
      if (row === void 0) {
        throw new Error(`Invalid row: ${y2}`);
      }
      row[x2] = this.currentPlayer;
    ...
    getWinner() {
      ...
      for (let y2 = 0; y2 < 3; y2++) {
        for (let x2 = 0; x2 < 3; x2++) {
          const cell = this.board[y2]?.[x2];
          if (cell !== null && cell !== void 0) {
            const score = this.boardScore[y2]?.[x2];
            if (score !== void 0) {
