...
(() => {
  ...
  var Node = class _Node {
    ...
    nextMove(game2) {
      const children2 = this.getChildren(game2);
      if (children2.length === 0) {
        throw new Error("Cannot get next move from a node with no children");
      }
      const shuffled = this.shuffle([...children2]);
      if (this.randomNode) {
  ...
  var MCTS = class {
    ...
    selectMove() {
      ...
      const children2 = this.rootNode.getChildren(this.game);
      if (children2.length === 0) {
        throw new Error("No possible moves available");
      }
      let bestChild = children2[0];
      if (bestChild === void 0) {
    ...
    getBestMove() {
      const children2 = this.rootNode.children;
    ...
    getMoveStats() {
      ...
      const player = this.game.getCurrentPlayer();
      return children2.filter((child) => child.move !== null && child.visits > 0).map((child) => {
  ...
  // web/src/tree-viz.ts
  function convertToD3Tree(node, formatMove2, maxDepth = 3) {
    const d3Node = {
      name: node.move === null ? "Root" : formatMove2(node.move),
      visits: node.visits,
      winRate: node.winRate
    };
    if (maxDepth > 0 && node.children.length > 0) {
      const sortedChildren = [...node.children].sort((a, b) => b.visits - a.visits).slice(0, 7);
      d3Node.children = sortedChildren.map(
        (child) => convertToD3Tree(child, formatMove2, maxDepth - 1)
      );
    }
    return d3Node;
  }
  function getColorByVisits(visits, maxVisits) {
    if (maxVisits === 0)
      return "#64748b";
    const ratio = visits / maxVisits;
    if (ratio > 0.6) {
      const intensity = (ratio - 0.6) / 0.4;
      return rgb_default("#22c55e", "#14b8a6")(intensity);
    } else if (ratio > 0.2) {
      const intensity = (ratio - 0.2) / 0.4;
      return rgb_default("#f59e0b", "#22c55e")(intensity);
    } else {
      const intensity = ratio / 0.2;
      return rgb_default("#ef4444", "#f59e0b")(intensity);
    }
  }
  function findMaxVisits(node) {
    let max2 = node.visits;
    if (node.children) {
      for (const child of node.children) {
        max2 = Math.max(max2, findMaxVisits(child));
      }
    }
    return max2;
  }
  function renderD3Tree(container, treeData) {
    container.innerHTML = "";
    if (treeData.visits === 0) {
      container.innerHTML = '<p style="color: var(--text-muted); text-align: center; padding: 20px;">No simulations yet...</p>';
      return;
    }
    ...
    const svg = select_default2(container).append("svg").attr("width", width).attr("height", height).attr("viewBox", `0 0 ${width} ${height}`).attr("style", "max-width: 100%; height: auto;");
    const g = svg.append("g").attr("transform", `translate(${margin.left},${margin.top})`);
    const treeLayout = tree_default().size([height - margin.top - margin.bottom, width - margin.left - margin.right]);
    const root2 = hierarchy(treeData);
    const treeNodes = treeLayout(root2);
    g.selectAll(".link").data(treeNodes.links()).enter().append("path").attr("class", "link").attr("fill", "none").attr("stroke", "#475569").attr("stroke-width", (d) => {
      const targetVisits = d.target.data.visits;
      return Math.max(1, Math.min(4, targetVisits / maxVisits * 4));
    }).attr("stroke-opacity", 0.6).attr(
      "d",
    ...
    }).attr("fill", (d) => getColorByVisits(d.data.visits, maxVisits)).attr("stroke", "#1e293b").attr("stroke-width", 2).style("filter", "drop-shadow(0 2px 4px rgba(0,0,0,0.3))");
    node.append("text").attr("dy", -15).attr("text-anchor", "middle").attr("fill", "#f8fafc").attr("font-size", "11px").attr("font-weight", "600").text((d) => d.data.name);
    node.append("text").attr("dy", 4).attr("text-anchor", "middle").attr("fill", "#1e293b").attr("font-size", "9px").attr("font-weight", "bold").text((d) => {
      const visits = d.data.visits;
      if (visits >= 1e3) {
    ...
    legend.append("text").attr("x", 0).attr("y", 0).attr("fill", "#94a3b8").attr("font-size", "10px").text("Visits:");
    const legendColors = [
    ...
    legendColors.forEach((item, i) => {
      legend.append("circle").attr("cx", 5 + i * 30).attr("cy", 15).attr("r", 5).attr("fill", item.color);
      legend.append("text").attr("x", 12 + i * 30).attr("y", 18).attr("fill", "#94a3b8").attr("font-size", "8px").text(item.label);
    });
  }
